...
class TestDemoExtrasViews:
    ...
    def test_demo_extras_renders_metadata(self, auth_client, staff_user, mocked_responses):
    ...
    def test_ajax_add_demo_extras_success(self, auth_client, staff_user, mocked_responses):
        ...
        dummy_tar = SimpleUploadedFile("new.tar.gz", b"archive contents", content_type="application/gzip")
    ...
    def test_ajax_add_demo_extras_failure_renders_500(self, auth_client, staff_user, mocked_responses):
        ...
        dummy_tar = SimpleUploadedFile("bad.tar.gz", b"corrupt", content_type="application/gzip")
    ...
    def test_ajax_delete_demo_extras_success(self, auth_client, staff_user, mocked_responses):
        ...
        response = auth_client.get("/cp2/demoExtras/ajax_delete_demo_extras?demo_id=100")
        assert response.status_code == 302
        assert response.url == "/cp2/demoExtras?demo_id=100"

    def test_ajax_delete_demo_extras_failure(self, auth_client, staff_user, mocked_responses):
...
class TestTemplateLinkingAndBlobDemo:
    def test_ajax_add_template_to_demo_authorized(self, auth_client, staff_user, mocked_responses):
    ...
    def test_ajax_add_template_to_demo_unauthorized(self, auth_client, mocked_responses):
    ...
    def test_ajax_remove_template_to_demo_authorized(self, auth_client, staff_user, mocked_responses):
    ...
    def test_ajax_edit_blob_demo_success(self, auth_client, staff_user, mocked_responses):
...
class TestArchiveExperimentViews:
    ...
    def test_show_archive_renders_experiments_and_pagination(self, auth_client, staff_user, mocked_responses):
        ...
        response = auth_client.get("/cp2/showArchive?demo_id=100&page=1&title=Demo%20Archive")
        assert response.status_code == 200
    ...
    def test_show_experiment_success(self, auth_client, staff_user, mocked_responses):
        ...
        mocked_responses.add(
            ...
            json={"id": "exp_123", "stdout": "Finished execution", "status": "completed"},
            status=200,
        )

        response = auth_client.get("/cp2/showExperiment?demo_id=100&experiment_id=exp_123")
        assert response.status_code == 200
    ...
    def test_show_experiment_not_found(self, auth_client, staff_user, mocked_responses):
        ...
        response = auth_client.get("/cp2/showExperiment?demo_id=100&experiment_id=exp_999")
        assert response.status_code == 200
        assert "error.html" in [t.name for t in response.templates]
        assert response.context["error_code"] == 404
        assert "Experiment not found" in response.context["message"]

    def test_ajax_delete_experiment_success(self, auth_client, staff_user, mocked_responses):
    ...
    def test_ajax_delete_experiment_failure(self, auth_client, staff_user, mocked_responses):
