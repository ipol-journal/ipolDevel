...
Each demo of IPOL offers to the user a set of default blobs (\textbf{b}inary \textbf{l}arge \textbf{o}bjects). Thus, the users are not forced to supply their own files for the executions of the algorithms. These default blobs can be tagged and linked to different demos.

The blobs module is composed of several Python modules. It is composed of the same technical stack as the rest of the rest of the IPOL modules: written in Python, with FastAPI, mounts a router for every endpoint, a database, and using templates for generating HTML responses for webservices destined to humans.

\subsection{Router}
Blobs router is mounted in ``app.py'' and depends on the Core module in order to run. This router will listen to all http requests  destined to blobs api.
