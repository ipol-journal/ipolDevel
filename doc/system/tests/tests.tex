...
The objective of the tests is to analyze automatically if the system fits the intended use. For that purpose the system is analyzed with two
...
Those tests are automatically executed in the integration and production environments every day by the crontab and every time a pull is made by 
...
environments, e.g. down modules, lack of disk memory, etc. Or every time a pull is made from the terminal.py script to check if the added changes do not
damage the system.
