...
\begin{itemize}
  ...
  \item ccache: to speed up recompilation by caching
  ...
  \item libeigen3-dev liblapack-dev libblas-dev: linear algebra
  \item libopenblas-base libopenblas-dev: allow numpy, scipy, and others to run some operations in parallel
  \item parallel: build and execute command lines from standard input in parallel
  ...
  \item libgsl2 libgsl-dev libgsl-dbg  libblas-dev libblas3 liblapack-dev liblapack liblapacke-dev liblapacke: numerical computations with GSL, BLAS, LAPACK
  ...
  \item sshfs: to mount filesystem across different machines using SSH.
  \item libgdal-dev
  \item gdal-bin
  \item python-gdal
  \item libavformat-dev
  \item libavcodec-dev
  \item libavdevice-dev
  \item libswscale-dev 
  \item libavresample-dev 
  \item libavfilter-dev 
  \item libavutil-dev
\end{itemize}

The following packages are needed in our of our private projects on satellite imaging:
\begin{itemize}
  \item libgeos-dev
\end{itemize}
