...
\miguel{Carlos: Add here an introduction which explains what is a reverse proxy, what is a WSGI, and what is an application server. And why they are needed, in general.}
...
The communication between the client and the Control Panel in local is direct, since the requests go directly to 
the port where the control panel is running. Django, in which is implemented the Control Panel, is in debug mode, so 
it is able to respond the requests from the client for the static files, without the need of a WSGI.
The diagram of this configuration is represented by Figure ~\ref{fi:local_architecture}.\\

\begin{figure}[!ht]
\centering
\includegraphics[width=0.4\columnwidth]{sysadmin/images/local.pdf}
\caption{Local \miguel{The caption needs to be more informative. For example: The communication between the client and the Django web server is direct in the local case. Also, add a final period at the end of the sentence.}}
\label{fi:local_architecture}
\end{figure}

\miguel{For the image labels, use the 'fig:' prefix, not 'fi:'}

The communication in the integration server adds a reverse proxy to make the redirections. This reverse proxy is implemented with
NGINX and allows the client to do the requests in the 80 port. That request is intercepted by NGINX and send to the 8000
port where Django is running. Just as in local, there is not need for a WSGI because Django is in debug mode and it is able to 
respond the request for static files. The diagram of this configuration is represented by Figure ~\ref{fi:integration_architecture}.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\columnwidth]{sysadmin/images/integration.pdf}
\caption{Integration \miguel{Add a more explicative caption. Add the final period.}} 
\label{fi:integration_architecture}
\end{figure}

In production is where is located the most complete connection. As in the integration server, a reverse proxy is needed to 
make the redirections to hide to the client in wich port is Django running. In addition, in the production server a WSGI is also
needed to provide all the static files since Django is not in debug mode and it is unable to do it by itself. This WSGI is 
implemented with Gunicorn and communicates with the NGINX through an UNIX \miguel{If the word begins with a U, you write 'a', not 'an'} socket. The diagram of this configuration is represented 
by Figure ~\ref{fi:production_architecture}.


\begin{figure}[!ht]
\centering
\includegraphics[width=1\columnwidth]{sysadmin/images/production.pdf}
\caption{Production \miguel{Add a more explicative caption. Add the final period.}}
\label{fi:production_architecture}
\end{figure}
...
\begin{itemize}
\item URL: \url{http://ipolcore.ipol.im/accounts/login/}
