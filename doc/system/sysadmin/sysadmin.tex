...
implemented with NGINX and is used to redirect request from port 80 to the Control Panel listening port.
\item \textbf{WSGI} is a specification for simple and universal interface between web servers and web applications or frameworks. In this case the
Gunicorn is the responsible of this purpose.
...
\caption{The direct communication between the client and the Django web server in the local server.}
...
respond the request for static files. The diagram of this configuration is represented by Figure ~\ref{fig:integration_architecture}.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\columnwidth]{sysadmin/images/integration.pdf}
\caption{The communication between the client and the Django web server with the reverse proxy in the integration server.}
...
implemented with Gunicorn and communicates with the NGINX through a UNIX socket. The diagram of this configuration is represented 
by Figure ~\ref{fig:production_architecture}.


\begin{figure}[!ht]
\centering
\includegraphics[width=1\columnwidth]{sysadmin/images/production.pdf}
\caption{The communication between the client and the Django web server with the reverse proxy and the WSGI in the production server.}
