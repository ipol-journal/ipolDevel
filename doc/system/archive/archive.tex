...
The services all connect to each database in a thread-safe way, instantiating its own connection when called, committing when done if there are modifications, or rolling back individual queries, if there is an error, and closing the connection. \\
...
Each experiments, and each blobs are defined individually, and linked to each other in the correspondence table, assuring a many-to-many connection. It is worth noting that the database does not save duplicates of the same blob. \\
...
The following lines are an example of using the archive. 
\begin{verbatim}
