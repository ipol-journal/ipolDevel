...
Inpainting controls are available in order to let the user draw on top of blobs. When adding an inpainting control to the inputs of a demo, the core module will create files according to the data the interface sends before actual execution of the demo code. Here's a brief explanation for each kind of control.
