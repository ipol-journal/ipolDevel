...
\subsection{Helpers}
This module acts as an interface for common utility methods like read and write to sessionStorage, make HTTP requests and 
...
The idea behind the archive experiment reconstruction is the same as execution recovery explained in the previous section. The user is able to reconstruct the experiment with the same inputs, parameters and results as the original experiment. The difference between execution recovery and archive experiment reconstruction is that the second one is not temporal. The experiment is stored in the database so users are able to reconstruct them anytime.
\bigbreak
