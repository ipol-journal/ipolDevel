...
\begin{itemize}
  ...
  As docker run does not accept relative paths, we use \$(pwd) to get back the currect directory: ipolDevel's repository root as always. If you are one
...
Which means you ran the \textbf{docker run} or the \textbf{docker start} command above.  
...
Create an \emph{authorized\_patterns.conf} file by copying the sample one.
\begin{lstlisting}[firstnumber=1,breaklines]
  $ cd ~/ipolDevel/ipol_demo/modules/config_common/
  $ cp authorized_patterns.conf.sample authorized_patterns.conf
\end{lstlisting}
...
Note: Don't worry if you see an error about a distribution not found for Django. It is about \textbf{cp2}
which is not of use today.

Now, we want to create a super user for cpanel. still with the \emph{venv} we just created, and
still in \emph{\~/ipolDevel/ipol\_webapp} :
\begin{lstlisting}[firstnumber=1,breaklines]
  (venv) $ python manage.py createsuperuser
\end{lstlisting}

Pick a username, an email address and a password as you wish.

\textbf{Important} : The only ling left to do is to download the staticData and db folders for the demoInfo
and blobs modules. For that, you will have to ask someone in the team.  
...
filled with the new files. Paths are relative to ipolDevel's repository root.
...
above. You can of course start cpanel in the backgroud if you want with : cpanel \&.  

You can also access the demos at \emph{http://127.0.0.1/demo/} (no need to have cpanel running for
that).
