...
Administrative scripts are available in the system for tasks such as backups, data cleanup, and continuous integration.Some of these are executed via cron.

\begin{itemize}

    \item \textbf{\texttt{backup.sh}} (\texttt{ipolDevel/sysadmin/backup.sh}): Creates backups of the \texttt{ipolDevel} directory and associated data (see Section~\ref{sec:backups}).

    \item \textbf{\texttt{cleanup.sh}} (\texttt{ipolDevel/sysadmin/cleanup.sh}): Removes temporary files older than one month from the system.

    \item \textbf{\texttt{pull.sh}} (\texttt{ipolDevel/ci\_tests/pull.sh}): Pulls the latest repository changes for continuous integration tasks.

    \item \textbf{\texttt{published\_codes\_compilation\_test.sh}} (\path{ipolDevel/ci_tests/published_codes_compilation_test.sh}): Executes compilation tests for published code artifacts.

    \item \textbf{\texttt{backup\_sync.sh}} (\texttt{ipolDevel/sysadmin/backup\_sync.sh}): Synchronizes backup data to a remote server.

    \item \textbf{\texttt{pdflatex.sh}} (\texttt{ipolDevel/ci\_tests/pdflatex.sh}): Checks whether the LaTeX documentation compiles successfully and sends an email if it fails.

    \item \textbf{\texttt{on\_reboot.sh}} (\texttt{ipolDevel/sysadmin/on\_reboot.sh}): Starts IPOL modules, Nginx, and shared services on system startup, and sends a notification email.
...
This section defines the active cron configuration of the system.

\subsection{Crontab}

Cron entries use the standard five-field scheduling format (minute, hour, day of month, month, day of week).
...
0 11 * * * /home/ipol/ipolDevel/ci_tests/published_codes_compilation_test.sh

# System crontab (/etc/crontab)
0 1 * * 4 root /bin/sh /root/backup_sync.sh
\end{verbatim}

\subsection{Notes}

\begin{itemize}
    \item Only active cron jobs are listed.
    \item Cron Jobs are defined in either the user crontab or the system crontab.
    \item Output redirection (e.g. \texttt{> /dev/null}) suppresses logs.
    \item See Section~2 for script definitions.
\end{itemize}

\section{GitHub deploy key}
The IPOL GitHub repository might be private. Thus, if any server needs to obtain the complete source code of the demo system, a \emph{deploy key} is needed. The deploy key is added to the GitHub configuration and each server needs to get the sources using the deploy key.
...
- Configuration of the email server to be able to send emails from any of the IPOL servers: configure with {\tt dpkg-reconfigure exim4-config} and be sure to choose the option to send email on the Internet, not local.
...
The server now uses Let’s Encrypt to provision and manage SSL certificates automatically using Certbot with the NGINX plugin.
\begin{enumerate}
    \item Issue and configure the certificate:
     
     This command automatically requests and installs a valid certificate as well as updates the NGINX configuration to use the certificate.
    \begin{verbatim}[language=Bash]
     sudo certbot --nginx -d ipolcore.ipol.im
    \end{verbatim}
    \item Validate configuration and restart Nginx:
    \begin{verbatim}[language=Bash]
     sudo nginx -t && sudo nginx -s reload
    \end{verbatim}
    \item Renew the Certificate
    \begin{verbatim}[language=Bash]
     sudo systemctl enable --now certbot.timer
    \end{verbatim}
    \item Verify auto-renewal is scheduled:
    \begin{verbatim}[language=Bash]
