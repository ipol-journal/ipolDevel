...
To run the Django server for the Control Panel application you need to make the migrations once per
...
To make it easier to use, the container has some predefined alias which can be used to run the Terminal or the Control Panel, among others:
 \begin{itemize}
 	\item {\tt terminal}: get into the IPOL Terminal tool directory and start it
 	\item {\tt cpanel}: launches the control panel at 127.0.0.1 on port 8000
 	\item {\tt cp2}: launches Control Panel 2 application at 127.0.0.1 on port 8000.
 	\item {\tt collectstatic}: collects all the static files for the control panel
...
You can now leave the IPOL's Terminal, move to the ipol\_webapp directory, create its venv, activate it, install its 
...
\textbf{Important}: the only thing left to do is to download the staticData and db folders for the core and blobs modules.
Then extract the content of these folders respectively in \emph{$./ipol\_demo/modules/blobs$}
and in \emph{$./ipol\_demo/modules/core$} so that both of them have a staticData and db folders
