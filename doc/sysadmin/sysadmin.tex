...
\section{Installing the required packages}
The system needs of several packages from the Debian Stable distribution and also from the official Python packages to work.

The list of apt-get packages in apt-get\_requirements.txt, and they can be installed with the following command:
\begin{verbatim}
xargs sudo apt-get install < apt-get_requirements.txt
\end{verbatim}

On the other hand, the list of Python packages which need to be installed with pip is in requirements.txt and can be installed with
\begin{verbatim}
...
\section{Docker}
In order to allow members of IPOL get a development envirorment, we create a docker image.
...
container before actually running the server. For this purpose there is a script under the Docker folder called migrations\_cp.sh.
 After the migrations run the Django server as you would anywhere else: python manage.py runserver 127.0.1.1:8000. 
...
\item "~/ipolDevel" : The context of the image. Should always be the root of the project IPOL.
\item -t ipol: Tag the image with the name ipol to reference it later.
\item -f : flag to specify a Dockerfile
\item ~/ipolDevel/docker/Dockerfile: Location of the Dockerfile defining the image.
\end{itemize}

\paragraph{Docker run}:\\
\begin{lstlisting}[firstnumber=1,breaklines]
$ docker run -it -v $(pwd):/home/ipol/ipolDevel -p 80:80 -h docker --add-host docker:127.0.1.1 --name ipol ipol
\end{lstlisting}
...
\item --add-host docker:127.0.1.1: adds an entry to /etc/hosts mapping 127.0.1.1 to the host
