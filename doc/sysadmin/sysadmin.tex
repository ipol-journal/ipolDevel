...
\ToDo{Explain what an incremental backup is.}
This script creates incremental backups and copies them to other servers, allowing to recover the data in the system in case of a fatal disk failure (say, the contents of the Archive). Every quarter of the year a level-0 backup (all the stored data, including temporary files) is performed. A level-1 backup is done every week (until the next quarter, when a new full level-0 backup will start), that will only contain the incremental changes with respect to the last week.

We use the following line to run the backup script once per week at 3:00h:

{\tt 0 3 * * 1 /home/ipol/ipolDevel/sysadmin/backup.sh}

\ToDo{Explain (with some examples) how to extract a backup.}

\ToDo{Explain more about why we're copying files from Core to DR1 and the{\tt ipolDevel/sysadmin/rsync.sh} script.}
