...
This script creates incremental backups and and replicates these backups in other servers so in the case of a total failure there are ways of recovering. Every quarter of the year there will be a level 0 backup which will backup the entire project and every week until the next quarter there is a level 1 backup with the incremental changes.

We use the following line to run the backup script once per week at 3:00h:

    {\tt 0 3 * * 1 /home/ipol/ipolDevel/sysadmin/backup.sh}
    
    \item \textbf{Rsync}: the script {\tt ipolDevel/sysadmin/rsync.sh} synchronizes backups from ipolcore to dr1.
