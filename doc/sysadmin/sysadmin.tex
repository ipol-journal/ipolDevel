...
This script creates incremental backups and copies them to other servers, allowing to recover the data in the system in case of a fatal disk failure (say, the contents of the Archive). An incremental backup is a kind of backup that will only add new changes with respect to the last backup, resulting in smaller sized backups and faster archiving.
...
There are problems with the extraction of incremental {\tt tar} backups when extracting all backup files on the same extraction folder. When extracting backups on the same folder using {\tt tar} some files will throw file substitution errors, most probably this happens because of an undocumented bug in the {\tt tar} command. In order to minimize extraction errors, the only way we have found which results in no errors consists of extracting every backup needed in a different folder and then mixing every folder with the first backup of the semester with the help of the {\tt rsync} command. This last step should be done in order of backup creation. Mixing files with rsync also has its caveats, after mixing the extracted files we occasionally end up with more files than orignally on the backup. Some removed files will still be there and will not be removed, this is not especially dangerous as we end up with more files than needed. This might happen because of a file permission problem or a bug in the {\tt rsync} command.

It is important to notice that backups are located at the home directory of the ipol user under the backups folder and are separated by yearly semesters folders, for example: 

{\tt /home/ipol/backups/<YEAR>-<SEMESTER>/backup\_<DATE>.tar.gz}

For every semester there will be several backup files, one for every week of the semester because of the current configuration of the cronjob at the time of writing. There will also be a snapshot file, this file is used by the tar command to keep a log of the files that have changed from one backup to another so it can backup files that change between one backup and the next. This snapshot is only used during the backup process, it will not be of any use during extraction but in case of corruption or if it is removed the tar command will not find it and will therefore create a level 0 backup. 

When restoring an incremental backup the destination path for the backups needs to be specified as a parameter or else it will overwrite the original path for the backed up files. For example:
...
\begin{enumerate}
  ...
  \item For a given date, extract every backup in the semester until the needed date to be restored. As previously stated, extract each backup on a separate folder. For each backup addapt the following example:
...
While making backups is a good way to be able to recover in case of data loss, it is even better to distribute copies of said backups to other servers to improve backup redundancy. Just as data can be corrupt or lost, servers have downtime and there is always the possibility of a server malfunction. Thus, in order to improve backups reliability we synchronize our backup folder with another server just in case of a catastrophe. Along with the backups script there is a script called backup\_sync.sh that automatically synchronizes the backup folder form one server to another thus making backups redundant. This is accomplished using rsync over ssh credentials.
