...
\textbf{Docker container stuck in a loop on start.}

The main reason this can happen is probably a broken NGINX configuration file preventing the container to boot up as it launches NGINX on startup. To solve this copy either an empty file into the container's volume, or better yet, a correct version of the file that broke it in the first place thus enabling NGINX to start using the following command:

\begin{verbatim}
docker cp correct_file_version ipol:/path_to_broken_nginx_file
\end{verbatim}
