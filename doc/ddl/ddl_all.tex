\section{The Demo Description Lines (DDL)}
...
forbid\_preprocess         & Forbids that the demo attempts any pre-processing to the input data. This is useful for noise-estimation demos for instance, or any other that could be negatively affected by pre-processing. In case the system needs to do it (say, an input image is larger than the maximum allowed) but it has been forbidden, the algorithm will not be run and instead an error message will be shown & no \\ \hline
...
\subsubsection{ \emph{numeric} type}
...
The gallery\_new type uses the gallery class to display images. Its parameters are:
...
It displays a gallery with a number of images that depend on a parameter.
...
%------ message  ------
\subsubsection{ \emph{message} type}
...
\hline \hline
 ...
 visible    & Javascript/JQuery boolean expression evaluated in the context 
              defined in \ref{sec:JavascriptEval}, that hides the result if
              evaluated to false (if undefined the result is visible)& no \\ \hline
