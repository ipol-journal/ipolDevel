...
\\ \hline
 ...
 timeout 	    & It specifies maximum time in seconds allows to execute the algorithm. If the execution takes longer than the specified time the system stops the execution. & no \\ \hline
...
\begin{figure}[h]
\centering
\tcbox[sharp corners, boxsep=0.0mm, boxrule=0.5mm, colback=white]{\includegraphics[scale=0.25]{Images/geojson_example.png}}
...
The demo will receive a GeoJSON file containing the coordinates as a list of geometric features.
For example, a selection made of two polygons of three and four vertices would be encoded as follows:
...
Note that a polygon of $N$ vertices is encoded as a list of $N+1$ coordinates, where the first equals the last. This is the GIS \href{https://www.ogc.org/standard/sfa/}{standard} to represent a topologically closed curve.
...
The result type \emph{three\_d} displays a 3D output such as .obj files in an interactive viewer.

\begin{longtable}{|>{\bf}L{\dimexpr 0.15\linewidth}|L{\dimexpr 0.7\linewidth}|c|}
\hline
 \centering {key}     & \centering {\bf description} & {\bf req} \tabularnewline 
\hline \hline
 type       & three\_d  & yes \\ \hline
 visible    & Javascript expression evaluated as a boolean. & no \\ \hline
 label      & HTML label. & yes \\ \hline
 contents   & A 3D filename to display. & yes \\ \hline
\caption{Properties of the \emph{three\_d} type in the results section.}
\end{longtable}
