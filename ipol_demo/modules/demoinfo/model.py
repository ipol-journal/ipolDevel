...
class  DemoDAO(object):
	...
	def list(self,is_ative=True):
		...
		try:
			if is_ative:
				self.cursor.execute('''SELECT editor_demo_id, title, abstract, zipURL, active, stateID, id, creation, modification  FROM demo WHERE active = 1 ''')
			else:
				self.cursor.execute('''SELECT editor_demo_id, title, abstract, zipURL, active, stateID, id, creation, modification  FROM demo WHERE active = 0 ''')
			self.conn.commit()
