...
import numpy as np
import tifffile
from ..errors import IPOLImageReadError
...
class Image(object):
    ...
    def __init__(self, data=None, buf=None, src=None):
        ...
        if data is not None:
            ...
            if mime_type == "image/tiff":
                ...
                if self.is_tiff_image():
                    self.data = (
                        self.reverse_channels_order()
                    )  # tifffile reads as RGB, OpenCV need BGR
        ...
        if self.data is None:
            raise IPOLImageReadError(
                f"Image read error, data={data} buf={buf}, src={src}"
            )
        self.ensure_data_shape()
    ...
    def crop(self, x=0, y=0, width=0, height=0):
        ...
        if width <= 0 or height <= 0 or x < 0 or y < 0:
            raise ValueError(
                "Crop, bad arguments x={}, y={}, x+width={}, y+height={} outside image ({}, {})".format(
    ...
    def convert_channels(self, dst_channels):
        ...
        if src_channels == 3 and dst_channels == 1:
            ...
            raise ValueError(
                "Channel conversion, {} > {} not yet supported.".format(
    ...
    def convert_depth(self, dst_depth):
        ...
        if not dst_dtype:
            raise ValueError(
                "Destination depth '{}' not suppported for conversion.".format(
                    dst_depth
                )
            )
        ...
        # int -> float
            ...
            raise ValueError(
                "Conversion from '{}' to '{}', not supported.".format(
                    src_dtype, dst_depth
                )
            )
    ...
    def remove_alpha(data):
        ...
        if num_channels == 2:
            ...
            back_color = (
                0.21 * back_color[0] + 0.72 * back_color[1] + 0.07 * back_color[2]
            )
