...
class Demo:
    ...
    def __init__(self, editorsdemoid, title, state, creation=None, modification=None):
...
class DemoDescriptionDAO:
    ...
    def add(self, ddl):
        ...
        """
        self.cursor.execute(
            """INSERT INTO demodescription (creation, DDL)
    ...
    def update(self, ddl, demo_id):
        ...
        self.cursor.execute(
            """UPDATE demo SET modification=? WHERE editor_demo_id=?""",
            (nowtmstmp, demo_id),
        )
    ...
    def read(self, demo_description_id):
        ...
        result = None
        self.cursor.execute(
            """SELECT  ddl  FROM demodescription WHERE id=?""",
...
class DemoDAO:
    ...
    def delete(self, editor_demo_id):
        ...
        """
        self.cursor.execute(
            "DELETE FROM demo WHERE demo.editor_demo_id=?", (int(editor_demo_id),)
        )
        self.conn.commit()
    ...
    def read(self, editor_demo_id):
        ...
        result = None
        self.cursor.execute(
            """SELECT  d.editor_demo_id, d.title, s.name, d.creation, d.modification
    ...
    def set_ssh_key(self, editor_demo_id, pubkey, privkey):
        """
        Stores ssh keys for a given demo.
        """
        self.cursor.execute(
            """
        UPDATE demo
    ...
    def get_ddl(self, editorsdemoid):
        ...
        self.cursor.execute(
            """SELECT ddl.DDL
             ...
             WHERE editor_demo_id = ?
             ORDER BY ddl.creation DESC LIMIT 1""",
            (int(editorsdemoid),),
        )
...
class AuthorDAO:
    ...
    def read(self, the_id):
        ...
        result = None
        self.cursor.execute(
            """SELECT  name, mail,id, creation FROM author WHERE id=?""", (int(the_id),)
        )
        self.conn.commit()
        row = self.cursor.fetchone()
        if row:
    ...
    def list(self):
        ...
        author_list = list()
        self.cursor.execute(
            """SELECT name, mail,id, creation FROM author ORDER BY id DESC """
        )
        self.conn.commit()
        for row in self.cursor.fetchall():
            a = Author(row[0], row[1], row[2], row[3])
...
class DemoEditorDAO:
    ...
    def read_editor_demos(self, editorid):
        ...
        demo_list = list()
        self.cursor.execute(
            """SELECT d.editor_demo_id, d.title, s.name, d.id, d.creation, d.modification
