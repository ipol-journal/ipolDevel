...
def test_tiff_depths():
    ...
    for src_depth in ("8i", "16i", "32i", "16f", "32f"):
        for dst_depth in (
            ...
            "16i",
            "32i",
        ):  # float as a destination is not strictly equal
...
def test_8i_extensions():
    ...
    for src_ext, src_mime in list(exts.items()):
        src_file = tempfile.NamedTemporaryFile(
            suffix=src_ext, prefix="ipol_", delete=True
        )
        ...
        for dst_ext, dst_mime in list(exts.items()):
            dst_file = tempfile.NamedTemporaryFile(
                suffix=dst_ext, prefix="ipol_", delete=True
            )
...
def test_convert_video_to_avi(video_blob_path, converter):
    ...
    assert (
        avi_video["width"] * avi_video["height"]
        == input_video["width"] * input_video["height"]
    )
